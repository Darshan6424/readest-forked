...
// --- SELF-HOST OVERRIDE -----------------------------------------------
// Self-hosted deployments have no Stripe/IAP wired up, so the JWT never
// carries a `plan` claim and every account would otherwise resolve to
// 'free' below, gating cloud sync, TTS caching, email-in, and the storage/
// translation quotas. Forcing 'pro' here unlocks all of it in one place,
// since every gate in the app (isCloudSyncAllowed, isTTSCacheAllowed,
// isEmailInPlan, getStoragePlanData, getTranslationQuota, and the account
// page's plan display) resolves through these two functions.
// Revert to the commented-out logic below if you wire up real billing.
