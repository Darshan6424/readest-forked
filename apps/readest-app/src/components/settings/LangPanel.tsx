...
import { TRANSLATED_LANGS, TRANSLATOR_LANGS, DEFAULT_GROQ_SETTINGS } from '@/services/constants';
...
const LangPanel: React.FC<SettingsPanelPanelProp> = ({ bookKey, onRegisterReset }) => {
  ...
  const { settings, setSettings, saveSettings, applyUILanguage, activeSettingsItemId, setActiveSettingsItemId } =
    useSettingsStore();
  ...
  const [groqApiKey, setGroqApiKey] = useState(
    settings.groq?.apiKey ?? DEFAULT_GROQ_SETTINGS.apiKey,
  );
  const [groqModel, setGroqModel] = useState(settings.groq?.model ?? DEFAULT_GROQ_SETTINGS.model);
  ...
  // Groq credentials are global (BYO API key), not per-book, so they live in
  // SystemSettings and save through setSettings/saveSettings rather than
  // saveViewSettings.
  const handleGroqApiKeyChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newSettings = {
      ...settings,
      groq: { ...(settings.groq ?? DEFAULT_GROQ_SETTINGS), apiKey: value },
    };
    setSettings(newSettings);
    saveSettings(envConfig, newSettings);
  };

  const handleGroqModelChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const value = event.target.value;
    setGroqModel(value);
    const newSettings = {
      ...settings,
      groq: {
        ...(settings.groq ?? DEFAULT_GROQ_SETTINGS),
        model: value || DEFAULT_GROQ_SETTINGS.model,
      },
    };
    setSettings(newSettings);
    saveSettings(envConfig, newSettings);
    ...
    <div className={clsx('my-4 w-full space-y-6')}>
      ...
      <BoxedList title={_('Translation')} data-setting-id='settings.language.translationEnabled'>
        ...
        {translationProvider === 'groq' && (
          <>
            <SettingsRow label={_('Groq API Key')} data-setting-id='settings.language.groqApiKey'>
              <SettingsInput
                type='password'
                autoComplete='off'
                value={groqApiKey}
                ...
                placeholder='gsk_...'
                aria-label={_('Groq API Key')}
              />
            </SettingsRow>
            <SettingsRow label={_('Groq Model')} data-setting-id='settings.language.groqModel'>
              <SettingsInput
                type='text'
                autoComplete='off'
                value={groqModel}
                onChange={handleGroqModelChange}
                placeholder={DEFAULT_GROQ_SETTINGS.model}
                aria-label={_('Groq Model')}
              />
            </SettingsRow>
          </>
        )}
